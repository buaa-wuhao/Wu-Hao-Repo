...
% see the LICENSE file included with this software
    ...
    %% METHODS
        ...
        %   See also controller/execute
            ...
            %% START CODE BLOCK %%
            %1.once found at obstacle, start follow wall
            %2.Once go into "follow wall",we will follow wall
            %untill making progress, then switch to go to goal 
            %3.In follow wall mode, we will not avoid objects even
            %distance is unsafe. 05/12/2018
...
%                 fprintf('now sliding right\n');
        ...
        function rc = progress_made(obj,state, robot)
            ...
            %% START CODE BLOCK %%
            distance = norm([x-obj.goal(1);y-obj.goal(2)]);
            %disp(distance);
            %disp(obj.d_prog);
            if (distance < obj.d_prog - 0.1)
                rc = true;
            end
